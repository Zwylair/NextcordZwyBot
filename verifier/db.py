...
from verifier.classes import ErrorOrganizer


def check_verifier_args(
        bot: Bot, chat: nextcord.TextChannel = None,
        hex_colour: str = None,
        banner_url: str = None
) -> ErrorOrganizer:
...
class VerifierBaseEmbedView(nextcord.ui.View):
    def __init__(
        ...
        self.out_embed.add_field(
            name='`(Эта часть сообщения не попадет в финальный вариант сообщения)` ⬇️',
    ...
    async def update_out_embed(
        ...
        title = title or self.out_embed.title
        desc = desc or self.out_embed.description
        colour = colour or self.out_embed.colour
        banner_url = banner_url or self.out_embed.image.url
        field_title = field_title or self.out_embed.fields[0].name
        field_text = field_text or self.out_embed.fields[0].value
    ...
    async def edit_embed_role(self, _: nextcord.ui.Button, interaction: nextcord.Interaction):
        ...
        if user_answer.role_mentions[0].position > bot_role.position:
            embed = nextcord.Embed(
                colour=0xEA5D62,
                title='Ошибка! 🪲',
                description='Указанная роль выше чем роль бота!'
            )
            await self.base_interaction.send(embed=embed, ephemeral=True)
            return
        ...
        await self.update_out_embed(
            field_title='`(Эта часть сообщения не попадет в финальный вариант сообщения) ⬇️`',
