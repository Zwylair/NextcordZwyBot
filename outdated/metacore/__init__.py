...
class MetacoreCog(nextcord.ext.commands.Cog):
    def __init__(self, bot: nextcord.ext.commands.Bot):
        self.bot = bot

    @nextcord.slash_command(name='create_event', description='Создать событие', guild_ids=[settings.METACORE_GUILD_ID, settings.TEST_GUILD_ID])
    ...
    @nextcord.slash_command(name='create_own_event', description='Создать своё событие', guild_ids=[settings.METACORE_GUILD_ID, settings.TEST_GUILD_ID])
    async def create_own_event(self, interaction: nextcord.Interaction):
        if not await check_for_event_creator_role(interaction):
            return
