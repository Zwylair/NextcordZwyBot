...
class RolesModerationCog(nextcord.ext.commands.Cog):
    ...
    @nextcord.slash_command(name='give_role', description='Выдать роль пользователю')
    async def give_role(self, interaction: nextcord.Interaction,
        ...
        for user_role in interaction.user.roles:
            if user_role.permissions.manage_roles:
    ...
    @nextcord.slash_command(name='revoke_role', description='Забрать роль у пользователя')
    ...
    @nextcord.slash_command(name='edit_role', description='Изменяет указанную роль. Если не желаете менять параметр, пропустите его')
    async def edit_role(self, interaction: nextcord.Interaction,
        ...
        for usr_role in interaction.user.roles:
            if usr_role.permissions.manage_roles:
    ...
    @nextcord.slash_command(name='move_role', description='Изменяет позицию роли в иерархии')
